...
export default function MessagesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-slate-900">Messagerie & Échanges</h2>
        <p className="text-sm text-slate-500 mt-1">Discussions directes avec vos locataires par contrat</p>
      </div>

      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden h-[680px] flex">
        {/* Liste des conversations */}
        <div className="w-80 border-r border-slate-200 flex flex-col bg-slate-50/50">
          <div className="p-4 border-b border-slate-200 bg-white">
            <h3 className="font-semibold text-sm text-slate-800">Discussions en cours</h3>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-slate-100">
            {conversations.map((c) => (
              <button
                ...
                className={`w-full text-left p-4 transition-colors flex items-start gap-3 ${
                  selectedContractId === c.contractId ? "bg-brand-50 border-l-4 border-brand-600" : "hover:bg-slate-100/70"
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-slate-500 truncate">{c.property?.title}</p>
            ...
            {conversations.length === 0 && (
              <p className="text-xs text-slate-400 p-6 text-center">Aucun contrat ou locataire actif.</p>
            )}
          </div>
        </div>
          ...
          {selectedContractId && activeDetails ? (
            <>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50/30">
                {loadingMessages && (
                  <p className="text-xs text-slate-400 text-center py-4">Chargement des messages...</p>
                )}
                {messages.map((m) => {
                    ...
                    <div key={m.id} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                      ...
                      <div
                        className={`max-w-md px-4 py-2.5 rounded-2xl text-xs leading-relaxed shadow-sm ${
                          isMe ? "bg-brand-600 text-white rounded-br-none" : "bg-white text-slate-800 border border-slate-200 rounded-bl-none"
                        }`}
                      >
                        {m.content}
                      </div>
              ...
              <form onSubmit={handleSend} className="p-4 border-t border-slate-200 bg-white flex gap-2">
                <input
                  ...
                  className="flex-1 text-xs border border-slate-300 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500"
                />
                <button
                  type="submit"
