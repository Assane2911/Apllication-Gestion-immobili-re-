...
export default function TenantMessagesPage() {
  ...
  if (loading) {
    return <p className="text-slate-500 text-sm">Chargement de la messagerie...</p>;
  }

  if (conversations.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-slate-200 p-8 text-center">
        <p className="text-slate-500 text-sm">Aucun contrat de location actif associé pour échanger des messages.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-slate-900">Échanges avec mon gestionnaire</h2>
        <p className="text-sm text-slate-500 mt-1">Discutez directement avec votre agence / bailleur</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden h-[600px] flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
          <div>
            <h3 className="font-bold text-slate-900 text-sm">Agence & Gestionnaire</h3>
