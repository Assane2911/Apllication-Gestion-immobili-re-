import type { TranslationSchema } from "./fr";

const pt: TranslationSchema = {
  common: {
    tips: {
      logout: "Termina a sua sessão neste dispositivo.",
    },
    payment: {
      returned: "Pagamento enviado. A confirmação chega dentro de instantes — esta página atualiza-se sozinha.",
      cancelled: "Pagamento cancelado. Nenhum valor foi cobrado.",
    },
    appName: "ImmoPlatform Pro",
    civility: {
      label: "Tratamento",
      none: "Não especificado",
      M: "Senhor",
      MME: "Senhora",
    },
    phoneField: {
      countryLabel: "Indicativo do país",
      chooseCountry: "Escolher o indicativo",
      willBeSaved: "Será guardado como: {{numero}}",
      incomplete: "Número incompleto: {{numero}}",
      missingCountry: "Este número não indica nenhum país. Escolha o indicativo para que possa receber os lembretes do WhatsApp.",
    },
    actions: {
      save: "Guardar",
      saving: "A guardar...",
      cancel: "Cancelar",
      delete: "Eliminar",
      edit: "Editar",
      close: "Fechar",
      back: "Voltar",
      confirm: "Confirmar",
      retry: "Tentar novamente",
      none: "Nenhum",
      view: "Ver",
    },
    status: {
      AVAILABLE: "Disponível",
      OCCUPIED: "Ocupado",
      MAINTENANCE: "Manutenção",
      ACTIVE: "Ativo",
      ENDED: "Terminado",
      TERMINATED: "Rescindido",
      PENDING: "Pendente",
      PAID: "Pago",
      LATE: "Atrasado",
      CANCELLED: "Cancelado",
      OPEN: "Aberto",
      IN_PROGRESS: "Em curso",
      RESOLVED: "Resolvido",
      REJECTED: "Rejeitado",
      DRAFT: "Rascunho",
      COMPLETED: "Finalizada",
      PUBLISHED: "Publicado",
      ARCHIVED: "Arquivado",
      NEW: "Novo",
      CONTACTED: "Contactado",
      VISITED: "Visitado",
      CONVERTED: "Convertido",
    },
    paymentMethods: {
      STRIPE: "Cartão (Stripe)",
      PAYDUNYA: "PayDunya",
      BANK_TRANSFER: "Transferência",
      DEMO: "Demonstração",
    },
    theme: {
      tip: "Alterna entre a apresentação clara e a escura.",
      toLight: "Mudar para modo claro",
      toDark: "Mudar para modo escuro",
      toggleAria: "Alternar tema claro/escuro",
    },
    language: {
      label: "Idioma",
      selectAria: "Mudar de idioma",
    },
    pagination: {
      previous: "Anterior",
      next: "Próximo",
      pageOf: "Página {{page}} de {{totalPages}}",
      totalItems: "{{count}} no total",
    },
  },

  nav: {
    tips: {
      dashboard: "Visão geral: rendas recebidas, dívidas e vencimentos do mês.",
      properties: "Os seus imóveis: criar uma ficha, definir a renda, seguir a ocupação.",
      tenants: "Os seus inquilinos: contactos, documento de identidade e acesso ao portal.",
      owners: "Os proprietários para quem gere, e a respetiva comissão.",
      contracts: "Os contratos em curso e terminados, com assinatura eletrónica.",
      inspections: "Autos de entrada e de saída, com fotografias.",
      invoices: "As rendas emitidas: lembrar, receber, emitir os recibos.",
      listings: "Os seus anúncios publicados na montra pública.",
      leads: "Os pedidos recebidos pela montra, a qualificar e a acompanhar.",
      expenses: "Despesas e obras por imóvel, e a rentabilidade daí resultante.",
      fiscal: "Resumo anual das receitas e das despesas, pronto a declarar.",
      messages: "As suas trocas com os inquilinos, contrato a contrato.",
      issues: "Os problemas comunicados pelos inquilinos e o seu acompanhamento.",
      activityLog: "Quem fez o quê, e quando, no seu espaço.",
      agency: "Identidade da agência: contactos, logótipo e menções dos recibos.",
      subscription: "O seu plano, as suas faturas e o estado do seu período de teste.",
    },
    manager: {
      dashboard: "Painel",
      properties: "Imóveis",
      tenants: "Inquilinos",
      owners: "Proprietários",
      contracts: "Contratos",
      inspections: "Vistorias",
      invoices: "Pagamentos",
      listings: "Anúncios",
      leads: "Pedidos (CRM)",
      expenses: "Despesas & Rentabilidade",
      fiscal: "Balanço Fiscal",
      messages: "Mensagens",
      issues: "Incidentes",
      activityLog: "Registo de atividade",
      agency: "Minha Agência",
      subscription: "Minha Assinatura",
    },
    tipsTenant: {
      home: "O seu contrato, os seus documentos e os autos a assinar.",
      payments: "As suas rendas emitidas: pagar online e obter os seus recibos.",
      messages: "Escrever ao seu gestor, contrato a contrato.",
      issues: "Comunicar uma avaria com fotografia e acompanhar o tratamento.",
    },
    tipsOwner: {
      home: "O que os seus imóveis renderam este mês, apenas leitura.",
      crg: "O detalhe mês a mês: rendas, despesas, comissão e líquido a entregar.",
    },
    tenant: {
      home: "Minha habitação",
      payments: "Minhas rendas",
      messages: "Mensagens",
      issues: "Reportar um problema",
    },
    owner: {
      home: "Painel",
      crg: "Relatório de gestão",
    },
    logout: "Terminar sessão",
    managerSpace: "Área do gestor",
    tenantSpace: "Área do inquilino",
    ownerSpace: "Área do proprietário",
    currencyLabel: "Moeda de pagamento",
  },

  auth: {
    login: {
      title: "Gestão Imobiliária",
      subtitle: "Inicie sessão na sua conta",
      email: "Email",
      emailPlaceholder: "voce@exemplo.com",
      password: "Palavra-passe",
      passwordPlaceholder: "••••••••",
      submit: "Entrar",
      submitting: "A entrar...",
      forgotPasswordLink: "Esqueceu a palavra-passe?",
      backToHome: "← Voltar ao início",
      showPassword: "Mostrar a palavra-passe",
      hidePassword: "Ocultar a palavra-passe",
      noAccountText: "Ainda não tem conta?",
      registerLink: "Criar uma conta",
      resendVerificationButton: "Reenviar email de confirmação",
      resendVerificationSending: "A enviar...",
      resendVerificationSent: "Email reenviado! Verifique a sua caixa de entrada (e os spams).",
      panel: {
        title: "A gestão de arrendamento, finalmente simples.",
        subtitle: "Recibos automáticos, assinatura eletrónica e pagamentos online — tudo num só lugar.",
        highlights: [
          "Recibos em PDF certificados gerados automaticamente",
          "Assinatura eletrónica dos contratos em poucos cliques",
          "Pagamentos online seguros (cartão, mobile money, transferência)",
          "Interface disponível em francês, inglês, português e espanhol",
        ],
        trialNote: "15 dias de teste grátis, sem cartão bancário.",
      },
    },
    register: {
      title: "Criar uma conta",
      subtitle: "15 dias de teste grátis, sem cartão bancário.",
      email: "Email",
      emailPlaceholder: "voce@exemplo.com",
      password: "Palavra-passe",
      passwordPlaceholder: "••••••••",
      passwordHint: "Mínimo de 8 caracteres",
      confirmPassword: "Confirmar palavra-passe",
      submit: "Criar a minha conta",
      submitting: "A criar conta...",
      mismatchError: "As duas palavras-passe não coincidem.",
      tooShortError: "A palavra-passe deve ter pelo menos 8 caracteres.",
      sentTitle: "Verifique o seu email",
      sentSubtitle: "Acabámos de lhe enviar um email de confirmação. Clique no link para ativar a sua conta (verifique também os spams).",
      backToHome: "← Voltar ao início",
      alreadyHaveAccount: "Já tem uma conta?",
      loginLink: "Iniciar sessão",
      legalPrefix: "Ao criar uma conta, aceita os nossos",
      legalTerms: "termos de utilização",
      legalAnd: "e a nossa",
      legalPrivacy: "política de privacidade",
      legalSuffix: ".",
      panel: {
        title: "A sua agência, pronta em 2 minutos.",
        subtitle: "Crie a sua conta de gestor e comece a gerir os seus imóveis hoje mesmo.",
        highlights: [
          "Recibos em PDF certificados gerados automaticamente",
          "Assinatura eletrónica dos contratos em poucos cliques",
          "Pagamentos online seguros (cartão, mobile money, transferência)",
          "Interface disponível em francês, inglês, português e espanhol",
        ],
        trialNote: "15 dias de teste grátis, sem cartão bancário.",
      },
    },
    forgotPassword: {
      backToLogin: "← Voltar à conexão",
      backToLoginButton: "Voltar à conexão",
      title: "Esqueceu a palavra-passe",
      subtitle: "Indique o seu email e enviamos-lhe um link para escolher uma nova.",
      submit: "Enviar link",
      submitting: "A enviar...",
      sentTitle: "Email enviado",
      sentSubtitle: "Se existir uma conta com este email, vai receber um link de reinicialização em breve. Verifique também os spams.",
    },
    resetPassword: {
      title: "Nova palavra-passe",
      subtitle: "Escolha uma nova palavra-passe para a sua conta.",
      newPassword: "Nova palavra-passe",
      confirmPassword: "Confirmar palavra-passe",
      submit: "Reinicializar palavra-passe",
      submitting: "A atualizar...",
      mismatchError: "As duas palavras-passe não coincidem.",
      tooShortError: "A palavra-passe deve ter pelo menos 8 caracteres.",
      missingToken: "Este link de reinicialização está incompleto ou inválido.",
      requestNewLink: "Pedir um novo link",
      doneTitle: "Palavra-passe atualizada",
      doneSubtitle: "A sua palavra-passe foi alterada com sucesso. A redirecionar para a conexão...",
    },
    verifyEmail: {
      verifying: "A verificar...",
      verifyingSubtitle: "Aguarde um momento, por favor.",
      successTitle: "Email confirmado!",
      successSubtitle: "A sua conta está ativa. A redirecionar...",
      errorTitle: "Link inválido ou expirado",
      errorSubtitle: "Este link de confirmação já não é válido. Pode pedir um novo na página de conexão.",
      missingToken: "Este link de confirmação está incompleto ou inválido.",
      backToLogin: "Voltar à conexão",
    },
    google: {
      orDivider: "ou",
      error: "A conexão com o Google falhou. Tenta novamente.",
      wrongRole: "A conexão com o Google é reservada às contas de gestor.",
    },
  },

  landing: {
    nav: {
      features: "Funcionalidades",
      vitrine: "Os nossos imóveis",
      receipts: "Recibos & Assinatura",
      pricing: "Preços",
      currencies: "Multi-Moedas",
      security: "Segurança",
      faq: "FAQ",
      goToSpace: "Aceder à minha área →",
      login: "Iniciar sessão",
      trial: "Teste grátis de 15 dias →",
    },
    badge: "Nova Versão 2.0 • 15 dias de teste grátis, sem compromisso",
    hero: {
      titleLine1: "A gestão de arrendamento,",
      titleLine2: "reinventada & automatizada.",
      subtitle:
        "Gere os seus recibos de renda em PDF certificado, faça assinar os contratos online, envie lembretes automáticos no dia 1 de cada mês e acompanhe a sua rentabilidade líquida em tempo real.",
      ctaTrial: "Iniciar o teste grátis de 15 dias →",
      ctaPricing: "Ver os preços",
    },
    features: {
      receipts: {
        title: "Recibos & Contratos em PDF Certificados",
        desc: "Descarregamento instantâneo dos recibos de renda com o carimbo da agência, número de série e menções legais assim que o pagamento é confirmado.",
      },
      signature: {
        title: "Assinatura Eletrónica Tátil",
        desc: "Faça assinar os seus contratos de arrendamento diretamente no smartphone ou tablet. Registo de data/hora e arquivo digital seguro.",
      },
      reminders: {
        title: "Alertas Automáticos no Dia 1 de Cada Mês",
        desc: "Lembretes programados no dia 1 de cada mês, indicando o prazo de pagamento até ao dia 5, com um link de pagamento seguro.",
      },
      dashboard: {
        title: "Painel & Estatísticas em Tempo Real",
        desc: "Acompanhe rapidamente a sua taxa de ocupação, as receitas recebidas, os pagamentos em atraso e o estado do seu património, com gráficos claros e atualizados em direto.",
      },
      expenses: {
        title: "Controlo de Despesas & Rentabilidade Líquida",
        desc: "Registe as suas despesas de manutenção, impostos e encargos, e veja automaticamente o seu fluxo de caixa real — receitas menos despesas — mês após mês.",
      },
      tenantPortal: {
        title: "Portal do Inquilino Dedicado",
        desc: "Os seus inquilinos pagam a renda, consultam os seus recibos, reportam uma ocorrência com foto e escrevem-lhe diretamente, sem chamadas nem papelada.",
      },
      security: {
        title: "Segurança & Conformidade RGPD",
        desc: "Isolamento estrito dos dados entre agências, palavras-passe encriptadas, ligações seguras e alojamento em França — conforme o RGPD desde a conceção.",
      },
      multiCurrency: {
        title: "Multi-Moedas",
        desc: "Exiba e receba as rendas na moeda da sua escolha, adaptada ao seu mercado e ao dos seus inquilinos.",
      },
      multiLanguage: {
        title: "Interface Multilingue",
        desc: "Francês, inglês, português e espanhol: cada utilizador escolhe o seu idioma de exibição num clique, sem qualquer configuração.",
      },
    },
    featuresSection: {
      title: "Tudo o que precisa para gerir os seus arrendamentos com tranquilidade",
      subtitle: "Uma única plataforma, do contrato ao recebimento, tanto para independentes como para agências.",
    },
    extras: [
      "💬 Mensagens integradas",
      "🔍 Pesquisa global instantânea",
      "🕒 Registo de atividade completo",
      "📊 Exportação contabilística CSV",
      "🏢 Marca branca para a sua agência",
      "🌙 Modo escuro",
    ],
    productTour: {
      title: "Descubra a plataforma em imagens",
      subtitle: "Uma interface pensada para ser rápida, tanto para si como para os seus inquilinos.",
      dashboard: {
        title: "Um painel claro e acionável",
        desc: "Receitas, ocupação, pagamentos em atraso e ocorrências: todas as suas prioridades do dia, reunidas num único ecrã.",
      },
      contracts: {
        title: "Contratos assinados em poucos cliques",
        desc: "Geração do contrato, assinatura tátil do inquilino, arquivo automático — sem impressora nem digitalizador.",
      },
      expenses: {
        title: "Uma rentabilidade sempre atualizada",
        desc: "Cada despesa registada recalcula instantaneamente o seu fluxo de caixa líquido, imóvel a imóvel.",
      },
      tenant: {
        title: "Uma experiência fluida para o inquilino",
        desc: "Pagamento online, histórico de recibos e mensagens diretas: os seus inquilinos deixam de lhe ligar por qualquer motivo.",
      },
    },
    forWho: {
      title: "Concebido para todos os perfis de gestão de arrendamento",
      subtitle: "Quer gira um estúdio ou várias centenas de imóveis, há um plano feito para si.",
      independent: {
        title: "Proprietário independente",
        desc: "Gira os seus poucos imóveis com toda a simplicidade: recibos automáticos, lembretes de pagamento e portal do inquilino, sem complexidade desnecessária.",
      },
      agency: {
        title: "Agência imobiliária",
        desc: "Assinatura eletrónica, controlo de rentabilidade e marca branca para apresentar uma imagem profissional aos seus clientes proprietários e inquilinos.",
      },
      enterprise: {
        title: "Gestor de grande património",
        desc: "Multiutilizadores, exportação contabilística automatizada e apoio prioritário para gerir centenas de imóveis sem perder visibilidade.",
      },
    },
    trust: {
      title: "Os seus dados, protegidos a sério",
      subtitle: "A segurança não é uma opção adicionada depois — faz parte da arquitetura do serviço.",
      items: [
        "Dados alojados em França (Paris), numa infraestrutura dedicada.",
        "Isolamento estrito dos dados entre agências: nenhum gestor consegue ver os imóveis, inquilinos ou pagamentos de outra agência.",
        "Palavras-passe encriptadas e ligações totalmente seguras em HTTPS.",
        "Política de privacidade conforme o RGPD, consultável a qualquer momento.",
      ],
      linkLabel: "Consultar a nossa política de privacidade →",
    },
    faq: {
      title: "Perguntas frequentes",
      items: [
        {
          q: "Quanto tempo demora a começar?",
          a: "Apenas alguns minutos: crie a sua conta, adicione o seu primeiro imóvel e aproveite imediatamente 15 dias de teste grátis, sem necessidade de cartão bancário.",
        },
        {
          q: "Os meus dados estão seguros?",
          a: "Sim: os seus dados são alojados em França, encriptados e estritamente isolados das outras agências que utilizam a plataforma. Veja a nossa política de privacidade para mais detalhes.",
        },
        {
          q: "Que meios de pagamento os meus inquilinos podem utilizar?",
          a: "Cartão bancário (Stripe), mobile money e cartão via PayDunya (Orange Money, Wave, Free Money, MTN...) para África francófona, ou transferência bancária clássica.",
        },
        {
          q: "Posso cancelar a minha subscrição a qualquer momento?",
          a: "Sim, sem compromisso. Pode cancelar a partir da sua área de gestão a qualquer momento; o acesso mantém-se ativo até ao final do período já pago.",
        },
        {
          q: "O serviço funciona fora de França?",
          a: "Sim: moeda de exibição personalizável, interface em 4 idiomas e meios de pagamento adaptados tanto a África francófona como ao contexto internacional.",
        },
      ],
    },
    finalCta: {
      title: "Pronto para modernizar a sua gestão de arrendamento?",
      subtitle: "Junte-se aos gestores que já simplificaram os seus recibos, lembretes e acompanhamento financeiro.",
      cta: "Iniciar o meu teste grátis de 15 dias →",
    },
    pricing: {
      title: "Planos simples e transparentes",
      subtitle: "Todos os planos incluem 15 dias de teste grátis, sem necessidade de cartão bancário.",
      perMonth: "/ mês",
      starter: {
        name: "Starter",
        desc: "Ideal para senhorios independentes.",
        features: [
          "Até 5 imóveis geridos",
          "Recibos em PDF certificados",
          "Lembretes automáticos no dia 1 do mês",
          "Portal do inquilino incluído",
        ],
        cta: "Iniciar o teste de 15 dias",
      },
      pro: {
        name: "Pro Agência",
        badge: "Recomendado",
        desc: "Para agências e gestores em crescimento.",
        features: [
          "Até 25 imóveis geridos",
          "✍️ Assinatura eletrónica tátil",
          "💰 Acompanhamento de despesas & rentabilidade",
          "🏢 Marca branca (logótipo & cabeçalho da agência)",
          "💬 Mensagens diretas integradas",
        ],
        cta: "Iniciar o teste grátis de 15 dias",
      },
      enterprise: {
        name: "Empresa",
        desc: "Para empresas de gestão e grandes carteiras de imóveis.",
        features: [
          "Imóveis ilimitados",
          "Suporte prioritário dedicado 24/7",
          "Múltiplos utilizadores & gestores",
          "Exportação contabilística automatizada",
        ],
        cta: "Iniciar o teste de 15 dias",
      },
    },
    footer: {
      rights: "© 2026 ImmoPlatform Pro. Todos os direitos reservados.",
      tagline: "Software de gestão de arrendamento imobiliário certificado e seguro.",
    },
  },

  components: {
    documentModal: {
      subtitle: "Pré-visualização oficial certificada & Descarregamento / Impressão",
      print: "Imprimir / Guardar como PDF",
      printFailed: "Não é possível imprimir a partir desta janela. Utilize o menu de impressão do seu navegador.",
      loading: "A carregar o documento oficial...",
    },
    signatureModal: {
      title: "✍️ Assinatura eletrónica do contrato",
      instructions:
        "Trace a sua assinatura com o dedo ou o rato no quadro abaixo. Ao confirmar, atesta a exatidão e a assinatura legal do contrato de arrendamento.",
      signHere: "Assine aqui",
      clear: "Limpar",
      validate: "Confirmar a minha assinatura",
      validating: "A confirmar...",
      errorRequired: "Por favor, assine no quadro antes de confirmar.",
    },
    deleteAccountModal: {
      title: "Eliminar definitivamente a minha conta",
      subtitle: "Esta ação é irreversível.",
      warningTitle: "Ao confirmar, tudo o que se segue será eliminado imediata e definitivamente:",
      warningProperties: "Todos os seus imóveis e anúncios",
      warningTenants: "Todas as suas fichas de inquilinos e proprietários",
      warningContracts: "Todos os seus contratos, faturas e recibos",
      warningDocuments: "Todos os documentos e fotos associados (documentos de identidade, contratos digitalizados, ocorrências reportadas)",
      confirmWord: "ELIMINAR",
      confirmLabel: "Escreva «{{word}}» para confirmar",
      passwordLabel: "Volte a introduzir a sua palavra-passe atual",
      googleLabel: "Confirme ligando-se novamente com o Google",
      googleHint: "A sua conta foi criada através do Google e não tem palavra-passe: ligue-se novamente para confirmar a sua identidade.",
      googleConfirmed: "Identidade Google confirmada.",
      confirmButton: "Eliminar definitivamente a minha conta",
      deleting: "A eliminar...",
    },
    globalSearch: {
      placeholder: "Pesquisar um inquilino, um imóvel, um contrato...",
      clearAria: "Limpar a pesquisa",
      searching: "A pesquisar...",
      noResults: "Nenhum resultado para «{{query}}»",
      types: {
        tenant: "Inquilino",
        property: "Imóvel",
        contract: "Contrato",
        invoice: "Fatura",
      },
    },
    notificationBell: {
      tip: "Os seus alertas: rendas em atraso, contratos a terminar, incidentes abertos.",
      ariaLabel: "Notificações",
      title: "Notificações",
      itemCount: "{{count}} elemento(s) a exigir a sua atenção",
      allCaughtUp: "Tudo em dia 👍",
    },
    trialBanner: {
      expiredTitle: "Período de teste expirado",
      expiredDesc:
        "O seu período de teste de 15 dias terminou. Subscreva um plano para continuar a usar a aplicação.",
      unlockCta: "Desbloquear a minha conta →",
      trialBadge: "Modo de Teste de 15 Dias",
      lastDay: "Último dia do seu teste!",
      oneDayLeft: "Resta-lhe 1 dia de teste",
      daysLeft: "Restam-lhe {{count}} dias de teste grátis",
      trialDesc: "Aproveite todas as funcionalidades Pro sem compromisso. Pode ativar a sua assinatura a qualquer momento.",
      chooseOfferCta: "Escolher o meu plano ✨",
      activePlan: "Assinatura <1>Plano {{plan}}</1> ativa",
      manageSubscription: "Gerir a minha assinatura",
    },
    managerLayout: {
      subtitle: "Área do gestor",
      trialFree: "Teste grátis",
      closeMenuAria: "Fechar o menu",
      openMenuAria: "Abrir o menu",
    },
    tenantLayout: {
      title: "Área do inquilino",
    },
    ownerLayout: {
      title: "Portal do proprietário",
    },
    adminLayout: {
      title: "Administração da plataforma",
    },
  },

  manager: {
    dashboard: {
      title: "Painel",
      loadingHint: "A carregar os indicadores principais...",
      liveSystem: "Sistema em direto",
      overviewSubtitle: "Visão geral e desempenho da sua carteira de imóveis",
      errorTitle: "Erro ao carregar o painel",
      stats: {
        properties: "Imóveis",
        propertiesHint: "{{count}} disponível(is)",
        occupancy: "Taxa de ocupação",
        occupancyHint: "{{count}} habitação(ões) arrendada(s)",
        tenants: "Inquilinos ativos",
        tenantsHint: "Registados na agência",
        contracts: "Contratos de arrendamento ativos",
        contractsHint: "Contratos atualmente válidos",
        revenue: "Receita recebida (mês)",
        revenueHint: "Renda esperada: {{amount}}",
        lateInvoices: "Faturas em atraso",
        lateInvoicesHintYes: "Avisos de cobrança a enviar",
        lateInvoicesHintNo: "Nenhum pagamento em falta",
        openIssues: "Incidentes abertos",
        openIssuesHintYes: "Intervenções necessárias",
        openIssuesHintNo: "Todos os imóveis OK",
        maintenance: "Imóveis em manutenção",
        maintenanceHint: "Obras & renovação",
      },
      charts: {
        revenueVsExpenses: "Receitas vs Despesas (Últimos meses)",
        noRevenueData: "Ainda não há pagamentos ou despesas registados neste período.",
        currentMonth: "Mês atual",
        tooltipRevenue: "Receita recebida",
        tooltipExpense: "Despesas & Obras",
        legendRevenue: "Receitas",
        legendExpense: "Despesas",
        statusChart: "Estado da carteira de imóveis",
        noProperties: "Nenhum imóvel registado ainda.",
        propertiesCount: "{{count}} imóvel(is)",
      },
    },
    properties: {
      title: "Imóveis",
      count: "{{count}} imóvel(is) registado(s)",
      addBtn: "+ Adicionar um imóvel",
      formTitleEdit: "Editar imóvel",
      formTitleNew: "Novo imóvel",
      fields: {
        title: "Título",
        address: "Morada",
        surface: "Área (m²)",
        rent: "Renda mensal",
        status: "Estado",
        image: "Imagem",
        description: "Descrição",
        owner: "Proprietário (opcional)",
        noOwner: "Nenhum proprietário",
      },
      perMonth: "/ mês",
      emptyTitle: "Ainda não há imóveis",
      emptyDesc: "Adicione o seu primeiro imóvel (com foto, área e renda) para começar a gerir o arrendamento.",
      confirmDelete: 'Eliminar o imóvel "{{title}}"?',
    },
    tips: {
      contractSign: "Abre o painel de assinatura: a sua rubrica é aposta ao contrato e datada.",
      contractViewLease: "Mostra o contrato gerado, pronto a imprimir ou a enviar.",
      contractScan: "Junta ao contrato a fotografia ou o PDF do documento assinado em papel.",
      contractViewScan: "Abre o contrato em papel já digitalizado para este arrendamento.",
      contractCreate: "Abre o formulário de um novo contrato: imóvel, inquilino, renda e datas.",
      inspectionSave: "Guarda sem bloquear: ainda poderá corrigir.",
      inspectionSign: "Abre o painel de assinatura para validar o auto do lado da agência.",
      inspectionView: "Mostra o relatório completo, fotografias incluídas.",
      inspectionEdit: "Retoma o auto enquanto não estiver finalizado.",
      tenantPortal: "Abre a criação de um acesso ao portal para este inquilino.",
      tenantCreate: "Abre o formulário de uma nova ficha de inquilino.",
      issueStatus: "Passa o incidente a este estado e informa o inquilino.",
      leadStatus: "Classifica o pedido nesta fase do seu acompanhamento comercial.",
      expenseExportCsv: "Descarrega todas as despesas do filtro em formato folha de cálculo.",
      expenseReport: "Produz um relatório financeiro do período escolhido, pronto a enviar.",
      expenseCreate: "Abre o formulário de uma nova despesa a imputar a um imóvel.",
      fiscalGrandLivre: "Gera o livro razão do ano: todos os lançamentos, por ordem.",
      crgExport: "Produz o relatório de gestão do mês, para enviar ao proprietário.",
      propertyEdit: "Altera a ficha do imóvel: renda, área, proprietário, moeda.",
      propertyCreate: "Abre o formulário de um novo imóvel a gerir.",
      listingEdit: "Altera o anúncio tal como aparece na montra pública.",
      listingCreate: "Redige um anúncio e publica-o na sua montra pública.",
      ownerCreate: "Abre o formulário de um novo proprietário para quem gere.",
      invoiceSendMonthly: "Envia de uma vez um lembrete a TODOS os inquilinos com a renda em atraso, por email e WhatsApp.",
      agencyDeleteAccount: "Abre o procedimento de encerramento definitivo da agência e de todos os seus dados.",
      agencyLogoutAll: "Termina de imediato a sessão em todos os seus dispositivos. Terá de entrar novamente.",
      inspectionFinalize: "Bloqueia o auto e abre-o à assinatura. Deixa de ser alterável.",
      inspectionDelete: "Elimina o auto e as suas fotografias. Nada será recuperável depois.",
      contractRenew: "Cria um novo contrato a seguir a este, nas mesmas condições, e encerra o atual.",
      contractTerminate: "Rescinde o contrato antes do termo. As rendas seguintes cessam.",
      contractEnd: "Deixa o contrato extinguir-se no termo, sem renovação.",
      contractDelete: "Elimina o contrato e o seu histórico. Impossível se houver faturas associadas.",
      expenseDelete: "Elimina a despesa. A rentabilidade do imóvel e o resumo fiscal são recalculados.",
      listingDelete: "Retira o anúncio da montra pública e elimina a sua ficha.",
      propertyDelete: "Elimina o imóvel. Impossível se ainda houver um contrato associado.",
      ownerInvite: "Envia ao proprietário uma ligação de convite para o seu espaço, válida alguns dias.",
      subscribe: "Valida o plano escolhido e desencadeia o pagamento do primeiro período.",
      tenantIdDocument: "Abre o documento de identidade num novo separador, através de uma ligação temporária.",
      tenantEdit: "Altera a ficha: tratamento, nome, telefone, email, documento de identidade.",
      tenantExport: "Descarrega tudo o que o Serviço detém sobre este inquilino (direito de acesso RGPD).",
      tenantAnonymize: "Apaga definitivamente o que o identifica. As faturas e recibos são conservados.",
      tenantDelete: "Elimina a ficha. Impossível se houver um contrato associado.",
      ownerEdit: "Altera a ficha: contactos, IBAN, taxa de comissão.",
      ownerDelete: "Elimina a ficha. Impossível se houver um imóvel associado.",
      invoiceReceipt: "Abre o recibo em PDF, pronto a enviar ou a imprimir.",
      invoiceRemind: "Envia um lembrete ao inquilino, por email e por WhatsApp.",
      invoiceMarkPaid: "Regista o pagamento e gera o recibo correspondente.",
    },
    tenants: {
      exportData: "Exportar os seus dados",
      anonymize: "Anonimizar",
      anonymized: "Anonimizado",
      confirmAnonymize:
        "Apagar os dados identificativos de {{name}}?\n\nO nome, o email, o telefone, o documento de identidade e o acesso ao portal serão apagados definitivamente. Os contratos, faturas e recibos são conservados, como exige a regulamentação contabilística — ficarão associados a uma ficha anónima.\n\nEsta ação é irreversível e deixará de poder contactá-lo a partir do Serviço.",
      title: "Inquilinos",
      count: "{{count}} inquilino(s) registado(s)",
      addBtn: "+ Adicionar um inquilino",
      formTitleEdit: "Editar inquilino",
      formTitleNew: "Novo inquilino",
      fields: {
        firstName: "Nome próprio",
        lastName: "Apelido",
        phone: "Telefone",
        email: "Email",
        idDocument: "Documento de identificação (imagem ou PDF)",
      },
      emptyTitle: "Ainda não há inquilinos",
      emptyDesc: "Adicione o seu primeiro inquilino (nome, telefone, email, documento de identificação) para poder associá-lo a um contrato.",
      confirmDelete: 'Eliminar o inquilino "{{name}}"?',
      table: {
        name: "Nome",
        phone: "Telefone",
        email: "Email",
        idDocument: "Documento de identificação",
        portal: "Portal",
      },
      viewIdDocument: "Ver",
      viewIdDocumentMobile: "Ver o documento de identificação",
      noIdDocument: "Sem documento de identificação",
      portalActive: "Ativado",
      portalActiveMobile: "Portal ativado",
      createAccess: "Criar acesso",
      createPortalTitle: "Criar acesso ao portal para {{name}}",
      loginId: "Utilizador: {{email}}",
      tempPassword: "Palavra-passe temporária",
      portalSuccess: "Acesso ao portal criado com sucesso. Comunique o email e a palavra-passe ao inquilino.",
    },
    owners: {
      title: "Proprietários",
      count: "{{count}} proprietário(s) registado(s)",
      addBtn: "+ Adicionar um proprietário",
      formTitleEdit: "Editar proprietário",
      formTitleNew: "Novo proprietário",
      fields: {
        firstName: "Nome próprio",
        lastName: "Apelido",
        companyName: "Razão social (opcional)",
        phone: "Telefone",
        email: "Email",
        address: "Morada",
        iban: "IBAN",
        bic: "BIC / SWIFT",
        managementFeeRate: "Taxa de comissão (%)",
        notes: "Notas",
      },
      emptyTitle: "Ainda não há proprietários",
      emptyDesc: "Adicione um proprietário (nome, email, telefone) para lhe poder associar imóveis e dar acesso ao seu portal.",
      confirmDelete: 'Eliminar o proprietário "{{name}}"?',
      table: {
        name: "Nome",
        phone: "Telefone",
        email: "Email",
        feeRate: "Comissão",
        portal: "Acesso ao portal",
      },
      portalActive: "Ativo",
      portalPending: "Convite enviado",
      portalNone: "Sem acesso",
      invite: "Convidar",
      resendInvite: "Reenviar convite",
      inviteSuccess: "Convite enviado com sucesso.",
      viewCrg: "Relatório",
    },
    inspections: {
      title: "Vistorias",
      count: "{{count}} vistoria(s)",
      addBtn: "+ Nova vistoria",
      formTitle: "Nova vistoria",
      fields: {
        contract: "Contrato relacionado",
        selectContract: "Selecionar um contrato...",
        type: "Tipo",
      },
      typeEntry: "Entrada",
      typeExit: "Saída",
      create: "Criar",
      table: {
        property: "Imóvel",
        tenant: "Inquilino",
        type: "Tipo",
        date: "Data",
        status: "Status",
        signatures: "Assinaturas",
        actions: "Ações",
      },
      statusDraft: "Rascunho",
      statusCompleted: "Finalizada",
      agency: "Agência:",
      tenantLabel: "Inquilino:",
      signed: "Assinado",
      sign: "Assinar",
      pendingSignature: "Pendente",
      edit: "Editar",
      finalize: "Finalizar",
      confirmFinalize: "Finalizar esta vistoria? O conteúdo (cômodos, medidores, chaves) não poderá mais ser editado depois.",
      viewReport: "Ver relatório",
      confirmDelete: "Excluir este rascunho de vistoria?",
      emptyTitle: "Nenhuma vistoria ainda",
      emptyDesc: "Crie uma vistoria de entrada ou saída a partir de um contrato de arrendamento existente.",
      editTitle: "Edição da vistoria",
      roomsTitle: "Cômodos",
      addRoom: "+ Adicionar um cômodo",
      roomName: "Nome do cômodo",
      roomCondition: "Estado",
      roomNotes: "Observações",
      noRooms: "Nenhum cômodo registrado",
      conditionGood: "Bom estado",
      conditionFair: "Estado regular",
      conditionPoor: "Mau estado",
      metersTitle: "Leitura dos medidores",
      meterElectricity: "Eletricidade",
      meterWater: "Água",
      meterGas: "Gás",
      keysTitle: "Chaves e acessos entregues",
      addKey: "+ Adicionar",
      keyLabel: "Descrição",
      keyQuantity: "Qtd.",
      noKeys: "Nenhuma chave registrada",
      generalComments: "Observações gerais",
      removeRow: "Remover",
    },
    contracts: {
      title: "Contratos de Arrendamento",
      count: "{{count}} contrato(s) de arrendamento em gestão",
      addBtn: "+ Novo contrato",
      formTitle: "Novo contrato",
      fields: {
        property: "Imóvel",
        selectProperty: "Selecionar um imóvel",
        tenant: "Inquilino",
        selectTenant: "Selecionar um inquilino",
        rent: "Renda mensal",
        deposit: "Depósito de garantia",
        startDate: "Data de início",
        endDate: "Data de fim",
      },
      create: "Criar o contrato",
      confirmDelete: "Eliminar este contrato e todas as suas faturas associadas?",
      confirmRenew: "Renovar este contrato por mais 12 meses (a partir de {{date}})?",
      table: {
        property: "Imóvel",
        tenant: "Inquilino",
        rent: "Renda",
        duration: "Duração",
        signatures: "Assinaturas",
        status: "Estado",
        actions: "Ações",
      },
      overdue: "Vencido",
      endsInDays: "Termina em {{count}} dia(s)",
      agency: "Agência:",
      tenantLabel: "Inquilino:",
      signed: "Assinado",
      sign: "Assinar",
      pendingSignature: "Pendente",
      leasePdf: "Contrato PDF",
      scanPaperContract: "Digitalizar / Anexar contrato em papel",
      scanContractShort: "Scan papel",
      viewScannedContract: "Ver digitalização em papel",
      scannedLeaseAttached: "Contrato em papel digitalizado disponível",
      uploadingScan: "Enviando digitalização...",
      scanSuccess: "Contrato em papel digitalizado guardado com sucesso!",
      scanModalTitle: "Digitalização do contrato de arrendamento em papel",
      scanModalDesc: "Tire uma fotografia do contrato assinado com a câmara ou selecione um ficheiro (PDF, PNG, JPEG)",
      scanButtonCapture: "Tirar uma fotografia ou importar um ficheiro",
      renew: "Renovar (12 meses)",
      renewShort: "Renovar",
      doNotRenew: "Não renovar",
      terminate: "Rescindir",
      leaseTitle: "Contrato - {{property}}",
      leaseDocTitle: "Contrato de Arrendamento - {{property}}",
    },
    invoices: {
      title: "Pagamentos de Renda",
      subtitle: "Acompanhamento das mensalidades e recibos oficiais em PDF",
      filterAll: "Todas as faturas",
      filterByStatus: "Filtrar por estado",
      sendMonthlyAlerts: "Enviar alertas do dia 1 do mês",
      sendingMonthly: "A enviar...",
      confirmSendMonthly:
        "Enviar imediatamente o aviso de vencimento a todos os inquilinos para o mês atual (prazo de pagamento: dia 5)?",
      infoBannerTitle: "Automatização dos alertas mensais (dia 1 de cada mês)",
      infoBannerBody:
        "Um email de aviso de vencimento é <1>enviado automaticamente a cada inquilino no dia 1 do mês às 8h00</1>, lembrando-o de pagar a renda <1>até ao dia 5 do mês, o mais tardar</1>.",
      table: {
        period: "Período",
        tenant: "Inquilino",
        property: "Imóvel",
        amount: "Montante",
        dueDate: "Vencimento",
        method: "Método",
        status: "Estado",
        actions: "Ações",
      },
      dueDateLimit: "(Prazo até dia 5)",
      dueDatePrefix: "Vencimento: {{date}}",
      receiptPdf: "Recibo PDF",
      remind: "Lembrar",
      sendingReminder: "A enviar...",
      markPaid: "Marcar como pago",
      markingPaid: "A registar...",
      confirmMarkPaid:
        "Marcar a renda de {{name}} como paga ({{amount}})?\n\nO recibo será enviado de imediato por email ao inquilino, e esta ação é DEFINITIVA: uma fatura paga não pode voltar a pendente.",
      noInvoicesForFilter: "Nenhuma fatura para este filtro",
      receiptTitle: "Recibo de renda - {{period}}",
    },
    expenses: {
      title: "Despesas & Rentabilidade Real",
      subtitle: "Acompanhamento de encargos, faturas de obras e cálculo automático do cash-flow líquido",
      exportCsv: "Exportação Contabilística CSV",
      exportingCsv: "A exportar...",
      addExpense: "+ Registar uma despesa",
      categories: {
        MAINTENANCE: "🛠️ Manutenção & Obras",
        TAX: "🏛️ Imposto Predial",
        INSURANCE: "🛡️ Seguro do Proprietário",
        SYNDIC: "🏢 Condomínio / Administração",
        OTHER: "📦 Outro Encargo",
      },
      categoriesShort: {
        MAINTENANCE: "🛠️ Manutenção & Obras",
        TAX: "🏛️ Imposto Predial",
        INSURANCE: "🛡️ Seguro do Proprietário",
        SYNDIC: "🏢 Condomínio / Administração",
        OTHER: "📦 Outro encargo",
      },
      stats: {
        revenueCollected: "Rendas Recebidas",
        revenueHint: "{{count}} recibo(s) pago(s)",
        totalExpenses: "Total de Despesas & Obras",
        expensesHint: "{{count}} fatura(s) registada(s)",
        netCashFlow: "Cash-Flow Líquido Real",
        netCashFlowPositive: "Lucro líquido positivo",
        netCashFlowNegative: "Défice temporário",
      },
      filterByProperty: "Filtrar por imóvel:",
      allProperties: "Todos os imóveis",
      expenseLines: "{{count}} linha(s) de despesa",
      fullReportLabel: "Relatório financeiro completo (receitas + despesas) de",
      to: "até",
      generateReport: "Gerar o relatório (CSV)",
      generatingReport: "A gerar...",
      table: {
        date: "Data",
        property: "Imóvel",
        category: "Categoria",
        title: "Descrição",
        amount: "Montante",
        notes: "Notas",
        action: "Ação",
      },
      noExpenses: "Ainda não há despesas registadas.",
      confirmDelete: 'Eliminar a despesa "{{title}}" ({{amount}})?',
      noExpensesToExport: "Nenhuma despesa para exportar.",
      modalTitle: "Registar uma despesa / fatura",
      fields: {
        property: "Imóvel em causa *",
        selectProperty: "Selecionar um imóvel",
        category: "Categoria *",
        invoiceDate: "Data da fatura",
        titleSupplier: "Descrição / Fornecedor *",
        titlePlaceholder: "Ex.: Substituição do esquentador",
        amount: "Montante com impostos *",
        notes: "Notas / Detalhes adicionais",
        notesPlaceholder: "Canalizador, orçamento n.º...",
        reportDateFrom: "Data de início do relatório",
        reportDateTo: "Data de fim do relatório",
      },
      saveExpense: "Guardar a despesa",
      csvHeaders: {
        date: "Data",
        property: "Imóvel",
        category: "Categoria",
        title: "Descrição",
        amount: "Montante",
        currency: "Moeda",
        notes: "Notas",
      },
    },
    fiscal: {
      title: "Balanço Fiscal & Contabilidade",
      subtitle: "Síntese anual das receitas, despesas e resultado líquido — base da sua declaração de rendimentos prediais",
      errorTitle: "Erro ao carregar o balanço fiscal",
      yearLabel: "Exercício",
      downloadGrandLivre: "Descarregar o Livro-Razão (CSV)",
      downloadingGrandLivre: "A gerar...",
      stats: {
        totalRevenue: "Rendas recebidas",
        totalRevenueHint: "Rendas pagas durante o exercício {{year}}",
        totalExpenses: "Despesas do exercício",
        totalExpensesHint: "Todas as categorias incluídas",
        netResult: "Resultado líquido",
        netResultPositive: "Lucro predial líquido",
        netResultNegative: "Défice predial líquido",
      },
      charts: {
        revenueVsExpenses: "Receitas vs Despesas por mês ({{year}})",
        noData: "Sem dados para este exercício.",
        tooltipRevenue: "Rendas recebidas",
        tooltipExpense: "Despesas & Obras",
        legendRevenue: "Receitas",
        legendExpense: "Despesas",
        categoryBreakdown: "Repartição das despesas",
        noCategoryData: "Nenhuma despesa registada neste exercício.",
      },
      bilanParBien: {
        title: "Balanço predial por imóvel",
        subtitle: "Receitas, despesas e resultado líquido de cada imóvel ao longo do exercício",
        table: {
          property: "Imóvel",
          currency: "Moeda",
          revenue: "Receitas",
          expense: "Despesas",
          net: "Resultado líquido",
        },
        empty: "Nenhum lançamento para este exercício.",
      },
    },
    crg: {
      title: "Relatório de Gestão (CRG)",
      subtitle: "Repasse mensal de {{ownerName}} — aluguéis recebidos, despesas deduzidas, comissão e valor líquido a repassar",
      errorTitle: "Erro ao carregar o relatório de gestão",
      backToOwners: "← Voltar aos proprietários",
      monthLabel: "Mês",
      yearLabel: "Ano",
      exportHtml: "Exportar HTML para impressão",
      feeRateHint: "Taxa de comissão: {{rate}}%",
      stats: {
        totalLoyers: "Aluguéis recebidos",
        totalCharges: "Despesas deduzidas",
        totalCommission: "Comissão da agência",
        totalNet: "Valor líquido a repassar",
      },
      table: {
        title: "Detalhamento por imóvel",
        property: "Imóvel",
        currency: "Moeda",
        loyers: "Aluguéis recebidos",
        charges: "Despesas deduzidas",
        commission: "Comissão",
        net: "Valor líquido a repassar",
      },
      empty: "Nenhum imóvel associado a este proprietário.",
    },
    listings: {
      title: "Anúncios & Vitrine Pública",
      count: "{{count}} anúncio(s)",
      addBtn: "+ Novo anúncio",
      viewLeads: "Ver pedidos recebidos →",
      viewVitrine: "Ver a montra pública",
      formTitleEdit: "Editar anúncio",
      formTitleNew: "Novo anúncio",
      fields: {
        title: "Título",
        type: "Tipo de anúncio",
        price: "Preço",
        currency: "Moeda",
        pricePeriod: "Periodicidade do preço",
        surface: "Área (m²)",
        rooms: "Número de divisões",
        location: "Localização",
        country: "País",
        noCountry: "Não indicado",
        status: "Estado",
        contactPhone: "Telefone de contacto",
        contactWhatsapp: "WhatsApp",
        contactEmail: "Email de contacto",
        image: "Foto",
        featured: "Destacar (em destaque na vitrine)",
        description: "Descrição",
      },
      types: {
        RENT: "Arrendamento",
        SALE: "Venda",
        PROMOTION: "Promoção imobiliária",
        LAND: "Terreno",
        OTHER: "Outro",
      },
      pricePeriods: {
        MONTH: "Por mês",
        ONE_TIME: "Preço único",
      },
      perMonth: "/ mês",
      featuredBadge: "Em destaque",
      emptyTitle: "Nenhum anúncio por enquanto",
      emptyDesc: "Publique um anúncio para que apareça na vitrine pública, consultável pelos seus potenciais clientes sem necessidade de conta.",
      confirmDelete: 'Eliminar o anúncio "{{title}}"?',
    },
    listingLeads: {
      title: "Pedidos de visita e informação (CRM)",
      subtitle: "Pedidos recebidos através da vitrine pública sobre os seus anúncios",
      backToListings: "← Voltar aos anúncios",
      filterByListing: "Filtrar por anúncio",
      allListings: "Todos os anúncios",
      filterByStatus: "Filtrar por estado",
      allStatuses: "Todos os estados",
      requestTypes: {
        VISIT: "Pedido de visita",
        INFO: "Pedido de informação",
      },
      preferredDate: "Data preferida:",
      notesLabel: "Nota interna (não visível pelo potencial cliente):",
      notesPlaceholder: "Ex.: Recontactado em 12/09, disponível aos fins de semana...",
      changeStatus: "Alterar o estado:",
      emptyTitle: "Nenhum pedido por enquanto",
      emptyDesc: "Os pedidos de visita ou informação submetidos pelos visitantes da vitrine pública aparecerão aqui.",
    },
    messages: {
      title: "Mensagens & Trocas",
      subtitle: "Conversas diretas com os seus inquilinos, por contrato",
      ongoingDiscussions: "Conversas em curso",
      conversationCount: "{{count}} conversa(s)",
      noMessageYet: "Nenhuma mensagem por enquanto",
      noActiveContracts: "Nenhum contrato ou inquilino ativo.",
      loadingMessages: "A carregar as mensagens...",
      you: "Você (Agência)",
      tenantParen: "{{name}} (Inquilino)",
      noMessagesInThread: "💬 Nenhuma mensagem nesta conversa. Comece a conversa com o seu inquilino abaixo.",
      writeMessage: "Escreva a sua mensagem...",
      send: "Enviar",
      sending: "...",
      selectDiscussion: "Selecione uma conversa à esquerda para começar a trocar mensagens.",
    },
    issues: {
      title: "Relatos de Incidentes & Fotos",
      subtitle: "Inspeção visual dos incidentes fotografados pelos inquilinos",
      allStatuses: "Todos os estados",
      filterByStatus: "Filtrar por estado",
      photoCount: "Foto {{current}}/{{total}}",
      responseLabel: "Nota de tratamento / Resposta ao inquilino:",
      responsePlaceholder: "Ex.: Canalizador contratado, intervenção prevista para terça-feira às 14h...",
      concernedProperty: "Imóvel em causa:",
      changeStatus: "Alterar o estado:",
      noIssuesForFilter: "Nenhum incidente para este filtro",
      photoAlt: "Foto do incidente {{index}}",
      enlargedPhotoAlt: "Foto ampliada",
    },
    activityLog: {
      title: "Registo de Atividade",
      subtitle: "Histórico das ações efetuadas nos seus imóveis, inquilinos, contratos e pagamentos",
      filterByType: "Filtrar por categoria",
      filters: {
        all: "Todas as categorias",
        property: "Imóveis",
        tenant: "Inquilinos",
        contract: "Contratos",
        invoice: "Faturas",
        issue: "Incidentes",
      },
      table: {
        when: "Quando",
        who: "Quem",
        category: "Categoria",
        detail: "Detalhe",
      },
      emptyTitle: "Ainda não há atividade",
      emptyDesc: "Cada ação (adição de um imóvel, renovação de contrato, pagamento validado...) aparecerá aqui com a data e o autor.",
    },
    agencySettings: {
      title: "🏢 Identidade da Agência & Marca Branca",
      subtitle: "Personalize os dados de contacto e as menções legais que aparecerão nos seus recibos e contratos oficiais",
      success: "Definições da agência atualizadas com sucesso! Os seus futuros recibos incluirão estas informações.",
      fields: {
        agencyName: "Nome comercial da agência *",
        agencyNamePlaceholder: "Ex.: Horizonte Imobiliário Prestígio",
        siretOrId: "N.º de registo comercial (NIF)",
        siretOrIdPlaceholder: "Ex.: 849 203 194 00012",
        email: "Email de contacto oficial *",
        emailPlaceholder: "contacto@minhaagencia.com",
        phone: "Telefone da agência",
        phonePlaceholder: "+351 210 000 000",
        address: "Morada da sede da agência",
        addressPlaceholder: "Avenida da Liberdade 100, 1250-096 Lisboa",
        bankSectionTitle: "Dados bancários",
        bankSectionHint: "Mostrados aos inquilinos que escolham a transferência bancária como forma de pagamento.",
        iban: "IBAN",
        ibanPlaceholder: "PT50 0002 0123 1234 5678 9015 4",
        bic: "BIC / SWIFT",
        bicPlaceholder: "CGDIPTPL",
        legalNotice: "Menções legais do rodapé dos recibos",
        legalNoticePlaceholder:
          "Ex.: Sociedade de gestão imobiliária, capital social de 50.000€. Licença profissional n.º...",
      },
      save: "Guardar as definições",
      saving: "A guardar...",
      security: {
        title: "Segurança",
        description:
          "Fecha todas as sessões abertas nos seus dispositivos, incluindo este. Utilize-o se deixou uma conta ligada algures ou perdeu um telemóvel. Terá de iniciar sessão novamente.",
        logoutAllButton: "Terminar sessão em todos os dispositivos",
        logoutAllPending: "A fechar as sessões…",
        logoutAllError: "Não foi possível fechar as sessões. Tente novamente dentro de instantes.",
      },
      retention: {
        title: "Dados que atingiram o prazo de conservação",
        description:
          "Os prazos de conservação recomendados (referencial CNIL de gestão de arrendamento) assinalam as fichas seguintes. A decisão é sua: só você sabe se um litígio em curso justifica mantê-las. Anonimizar apaga os dados identificativos e conserva contratos, faturas e recibos.",
        none: "Nenhum dado atingiu o seu prazo de conservação.",
        noBail: "Ficha sem qualquer contrato, aberta há mais de {{days}} dias",
        leaseEnded: "Último contrato terminado há mais de {{years}} anos",
        error: "Não foi possível carregar os prazos.",
        seeTenants: "Abrir a lista de inquilinos",
      },
      dangerZone: {
        title: "⚠️ Zona de perigo",
        description:
          "Eliminar a sua conta apaga imediata e definitivamente todos os dados da sua agência (imóveis, inquilinos, contratos, faturas, documentos). Esta ação é irreversível.",
        deleteButton: "Eliminar a minha conta",
      },
    },
    subscription: {
      title: "Assinatura & Planos SaaS",
      subtitle: "Gira o seu plano de gestão imobiliária e as suas opções de faturação.",
      currentStatus: "O seu estado atual",
      planActive: "Plano {{plan}} Ativo",
      trialStatus: "Período de teste grátis (15 dias)",
      expiredStatus: "Assinatura Expirada",
      paid: "Pago",
      trialRemaining: "Teste: restam {{count}} dia(s)",
      expired: "Expirado",
      trialEndsAt: "O seu período de teste termina em {{date}}.",
      nextDueDate: "Próximo vencimento em {{date}}.",
      choosePlanPrompt: "Escolha um plano abaixo para desbloquear ou manter o seu acesso.",
      evaluationTime: "Tempo de avaliação oferecido",
      days: "dias",
      monthlyBilling: "Faturação Mensal",
      annualBilling: "Faturação Anual",
      mostPopular: "Mais popular",
      perMonth: "/ mês",
      billedAnnually: "(faturado {{amount}} / ano)",
      unsupportedCurrencyNotice: "As subscrições ainda não são faturadas na sua moeda de visualização. Os seus planos são por isso apresentados abaixo em {{currency}}.",
      propertyCapExceeded: "Gere {{count}} imóveis, mas o seu plano permite {{max}}. Os imóveis existentes continuam acessíveis, mas não poderá adicionar mais enquanto não mudar para um plano superior.",
      includedInPlan: "Incluído no plano:",
      currentPlanBtn: "O seu plano atual",
      choosePlanBtn: "Escolher {{plan}}",
      subscribeTo: "Subscrição do plano {{plan}}",
      amountToPay: "Montante a pagar:",
      perMonthShort: "/ mês",
      perYearShort: "/ ano",
      paymentMethodLabel: "Método de pagamento",
      bankRefLabel: "Número ou referência da transferência:",
      bankRefPlaceholder: "Ex.: TRF-2026-08-01",
      bankDetails: {
        title: "Dados bancários da plataforma",
        iban: "IBAN",
        bic: "BIC / SWIFT",
        missing: "A plataforma ainda não indicou os seus dados bancários. Contacte o suporte antes de efetuar a transferência.",
        copy: "Copiar",
        copied: "Copiado!",
      },
      confirmAndActivate: "Confirmar e Ativar a Assinatura",
      processing: "A processar...",
      historyTitle: "Histórico das suas faturas de assinatura",
      historyTable: {
        date: "Data",
        plan: "Plano",
        cycle: "Ciclo",
        amount: "Montante",
        method: "Método",
        status: "Estado",
      },
      annual: "Anual",
      monthly: "Mensal",
      paymentMethods: {
        STRIPE: { label: "💳 Cartão bancário", hint: "Pagamento seguro por cartão via Stripe" },
        PAYDUNYA: { label: "🌍 PayDunya", hint: "Orange Money, Wave, Free Money, MTN, cartão bancário..." },
        BANK_TRANSFER: { label: "🏦 Transferência bancária", hint: "Validação em 24h por transferência" },
        DEMO: { label: "🧪 Modo demonstração", hint: "Ativação imediata para testes e validação" },
      },
    },
  },

  tenant: {
    tips: {
      exportData: "Descarrega tudo o que o Serviço detém sobre si, num ficheiro legível.",
      viewLease: "Mostra o seu contrato tal como foi gerado.",
      viewInspection: "Mostra o relatório completo do auto, fotografias incluídas.",
      viewReceipt: "Abre o recibo desta renda, pronto a imprimir.",
      choosePayment: "Mostra os meios de pagamento aceites para esta renda.",
      addPhoto: "Acrescenta uma fotografia ao incidente já comunicado.",
      pay: "Inicia o pagamento desta renda por este meio. O recibo segue assim que o pagamento for confirmado.",
      signContract: "Abre o painel de assinatura: a sua assinatura vincula o contrato e é datada.",
      signInspection: "Assina o auto. Depois não poderá ser acrescentada qualquer reserva.",
    },
    dashboard: {
      privacy: {
        title: "Os meus dados pessoais",
        description:
          "Pode obter a qualquer momento uma cópia de todos os dados detidos a seu respeito: a sua ficha, os seus contratos, as suas faturas, as suas ocorrências e as suas mensagens. Para corrigir ou apagar estes dados, contacte o seu gestor, que é o responsável.",
        download: "Descarregar os meus dados",
        downloading: "A preparar o ficheiro…",
      },
      noContract: "Nenhum contrato de arrendamento está associado à sua conta neste momento.",
      title: "A Minha Habitação & O Meu Contrato",
      monthlyRent: "Renda mensal",
      deposit: "Depósito de garantia",
      startDate: "Início do contrato",
      endDate: "Fim do contrato",
      signatureTitle: "Assinatura do contrato de arrendamento",
      signedOn: "Assinado por si em {{date}}",
      awaitingSignature: "A aguardar a sua assinatura digital",
      signMyLease: "Assinar o meu contrato",
      viewLeasePdf: "Ver o contrato em PDF",
      viewScannedLease: "Ver contrato em papel digitalizado",
      paperLeaseNotice: "O gestor também anexou o exemplar oficial em papel digitalizado.",
      unpaidWarning: "Tem {{count}} mensalidade(s) de renda pendente(s) de pagamento.",
      payNow: "Pagar agora →",
      leaseTitle: "Contrato - {{property}}",
      leaseDocTitle: "Contrato de Arrendamento - {{property}}",
      inspectionsTitle: "Vistorias",
      inspectionEntry: "Vistoria de entrada",
      inspectionExit: "Vistoria de saída",
      inspectionAwaitingCompletion: "Em preparação pelo seu gestor",
      inspectionAwaitingYourSignature: "Aguardando sua assinatura",
      inspectionSignedByYouOn: "Assinado por você em {{date}}",
      inspectionSignMine: "Assinar a vistoria",
      inspectionViewReport: "Ver relatório",
      inspectionDocTitle: "Vistoria - {{property}}",
    },
    invoices: {
      title: "As Minhas Rendas & Recibos",
      dueDate: "Vencimento: {{date}}",
      receiptPdf: "O Meu Recibo PDF",
      pay: "Pagar",
      chooseMethod: "Escolher este método",
      paying: "A processar...",
      bankRefPlaceholder: "Referência da transferência",
      noInvoices: "Nenhuma fatura por enquanto",
      receiptTitle: "Recibo de renda - {{period}}",
      bankDetails: {
        title: "Dados bancários da agência",
        iban: "IBAN",
        bic: "BIC / SWIFT",
        missing: "A sua agência ainda não configurou os dados bancários. Contacte-a antes de fazer a transferência.",
        copy: "Copiar",
        copied: "Copiado!",
      },
      methods: {
        STRIPE: { label: "💳 Cartão bancário", hint: "Pagamento seguro via Stripe" },
        PAYDUNYA: { label: "🌍 PayDunya", hint: "Orange Money, Wave, Free Money, MTN, cartão bancário..." },
        BANK_TRANSFER: { label: "🏦 Transferência bancária", hint: "Declare a sua transferência, validação pelo gestor" },
        DEMO: { label: "🧪 Modo demonstração", hint: "Pagamento simulado, confirmação imediata" },
      },
    },
    issues: {
      title: "Reportar um Incidente com Fotos",
      subtitle: "Fotografe o incidente para transmitir imediatamente as imagens à agência",
      newReport: "Novo relato",
      concernedHome: "Habitação em causa",
      issueTitle: "Título do incidente *",
      issueTitlePlaceholder: "Ex.: Fuga de água no lavatório, fechadura encravada...",
      description: "Descrição detalhada *",
      descriptionPlaceholder: "Explique o que aconteceu, a urgência e a localização exata na habitação...",
      photoLabel: "📷 Fotografar o incidente (Obrigatório)",
      photoHint: "💡 No smartphone, a sua câmara abre diretamente para captar o incidente.",
      photoReady: "Foto pronta para enviar",
      errorPhotoRequired: "Por favor, tire ou anexe uma foto do problema",
      submit: "📤 Transmitir o relato à agência",
      submitting: "A enviar o relato...",
      historyTitle: "Os Meus Relatos & Fotos",
      managerResponse: "Resposta da agência:",
      reportedOn: "Relatado em {{date}} às {{time}}",
      addPhoto: "Adicionar uma foto a este incidente",
      addPhotoTitle: "Tirar uma foto adicional:",
      validateAddPhoto: "Confirmar a adição da foto",
      addingPhoto: "A enviar...",
      noIssues: "Nenhum incidente relatado. Todos os seus equipamentos estão a funcionar!",
      photoAlt: "Foto do incidente {{index}}",
      enlargedPhotoAlt: "Foto ampliada",
    },
    messages: {
      title: "Trocas com o Meu Gestor",
      subtitle: "Converse diretamente com a sua agência / senhorio",
      loading: "A carregar as mensagens...",
      loadingMessages: "A carregar as mensagens desta conversa...",
      noActiveContract: "Nenhum contrato de arrendamento ativo associado para trocar mensagens.",
      headerTitle: "Agência & Gestor",
      housing: "Habitação: {{property}}",
      yourApartment: "O seu apartamento",
      online: "Em linha",
      you: "Você",
      manager: "Gestor",
      noMessagesYet: "👋 Ainda não tem trocas de mensagens. Faça uma pergunta ao seu gestor abaixo.",
      writeMessage: "Escreva a sua mensagem...",
      send: "Enviar",
      sending: "...",
    },
  },
  owner: {
    tips: {
      crgExport: "Mostra o relatório de gestão do mês, para imprimir ou guardar.",
    },
    dashboard: {
      title: "Meu painel financeiro",
      subtitle: "Acompanhe o aluguel recebido e pendente dos seus imóveis.",
      collectedThisMonth: "Recebido este mês",
      pendingThisMonth: "Pendente este mês",
      managementFeeRate: "Taxa de comissão da agência",
      propertiesTitle: "Meus imóveis",
      noProperties: "Nenhum imóvel está associado à sua conta ainda.",
      collected: "Recebido",
      pending: "Pendente",
      historyTitle: "Histórico (últimos 6 meses)",
      noHistory: "Nenhum aluguel recebido nos últimos 6 meses.",
    },
    crg: {
      title: "Meu Relatório de Gestão",
      subtitle: "Repasse mensal: aluguéis recebidos, despesas deduzidas, comissão e valor líquido a repassar.",
      monthLabel: "Mês",
      yearLabel: "Ano",
      exportHtml: "Exportar HTML para impressão",
      feeRateHint: "Taxa de comissão: {{rate}}%",
      stats: {
        totalLoyers: "Aluguéis recebidos",
        totalCharges: "Despesas deduzidas",
        totalCommission: "Comissão da agência",
        totalNet: "Valor líquido a repassar",
      },
      table: {
        title: "Detalhamento por imóvel",
        loyers: "Aluguéis recebidos",
        charges: "Despesas deduzidas",
        commission: "Comissão",
        net: "Valor líquido a repassar",
      },
      empty: "Nenhum imóvel associado a você ainda.",
      bankDetailsTitle: "Seus dados bancários cadastrados",
      noBankDetails: "Nenhum dado bancário cadastrado — entre em contato com sua agência.",
    },
  },
  admin: {
    nav: {
      tips: {
        dashboard: "A atividade da plataforma, todos os gestores incluídos.",
        transfers: "As transferências de subscrição a confirmar ou a rejeitar à mão.",
        settings: "Os dados bancários onde os gestores pagam a sua subscrição.",
      },
      dashboard: "Painel",
      transfers: "Transferências bancárias",
      settings: "Definições",
    },
    dashboard: {
      title: "Painel de controlo",
      subtitle: "Visão geral da plataforma, todos os gestores incluídos",
      errorTitle: "Não foi possível carregar o painel",
      stats: {
        managers: "Gestores",
        managersHint: "{{trial}} em teste · {{expired}} sem acesso",
        subscriptionActive: "Assinaturas pagas ativas",
        subscriptionActiveHint: "De {{total}} gestor(es) no total",
        mrr: "Receita recorrente mensal",
        mrrHint: "{{count}} assinatura(s) paga(s)",
        properties: "Imóveis geridos",
        propertiesHint: "Em todas as agências",
        tenants: "Inquilinos",
        tenantsHint: "Em todas as agências",
        activeContracts: "Contratos ativos",
        activeContractsHint: "Em todas as agências",
      },
      mrrByPlan: {
        title: "MRR por plano",
        empty: "Nenhuma assinatura paga ativa por enquanto.",
      },
      trialsEndingSoon: {
        title: "Testes a terminar em breve",
        subtitle: "A contactar antes do fim do teste gratuito (7 dias ou menos)",
        empty: "Nenhum teste termina nos próximos 7 dias.",
        manager: "Gestor",
        noAgencyName: "Sem nome de agência definido",
        daysLeft: "{{count}} dia(s) restante(s)",
        lastDay: "Último dia",
      },
    },
    subscriptions: {
      title: "Transferências bancárias pendentes",
      subtitle: "Assinaturas SaaS pagas por transferência bancária, aguardando confirmação manual",
      emptyTitle: "Nenhuma transferência pendente",
      emptyDescription: "Todas as assinaturas pagas por transferência bancária foram confirmadas.",
      table: {
        manager: "Gestor",
        plan: "Plano",
        cycle: "Ciclo",
        amount: "Valor",
        reference: "Referência",
        since: "Desde",
      },
      annual: "Anual",
      monthly: "Mensal",
      confirmButton: "Confirmar pagamento",
      confirming: "A confirmar...",
      confirmPrompt: "Confirmar a receção da transferência de {{amount}} de {{email}}? Isto ativará imediatamente a assinatura.",
      rejectButton: "Rejeitar",
      rejecting: "A rejeitar...",
      rejectPrompt: "Rejeitar o pedido de transferência de {{amount}} de {{email}}? Isto não ativa nenhuma assinatura e não pode ser desfeito.",
    },
    settings: {
      title: "Definições da plataforma",
      subtitle: "Dados bancários usados para receber as assinaturas pagas por transferência",
      success: "Dados bancários guardados",
      fields: {
        iban: "IBAN da plataforma",
        ibanPlaceholder: "Ex.: FR76 XXXX XXXX XXXX XXXX XXXX XXX",
        bic: "BIC / SWIFT",
        bicPlaceholder: "Ex.: XXXXFRPPXXX",
      },
      save: "Guardar",
      saving: "A guardar...",
    },
  },

  vitrine: {
    brand: "ImmoPlatform Pro — Anúncios",
    managerLogin: "Acesso gestor",
    title: "Encontre o seu próximo imóvel",
    subtitle: "Consulte os anúncios publicados pelas nossas agências parceiras — arrendamentos, vendas, terrenos e promoções imobiliárias.",
    filterByCountry: "Filtrar por país",
    allCountries: "Todos os países",
    filterByType: "Filtrar por tipo",
    allTypes: "Todos os tipos",
    empty: "Nenhum anúncio corresponde a estes critérios por enquanto.",
    roomsCount: "{{count}} divisão(ões)",
    leadForm: {
      title: "Contactar a agência",
      name: "Nome completo",
      email: "Email",
      phone: "Telefone",
      requestType: "Tipo de pedido",
      preferredDate: "Data de visita preferida",
      message: "Mensagem (opcional)",
      submit: "Enviar pedido",
      success: "O seu pedido foi enviado! A agência entrará em contacto consigo brevemente.",
    },
  },
};

export default pt;
